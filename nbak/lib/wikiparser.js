...
wikiparser.parseWikiData = function (wikiData, wikiDataType){
    if (wikiDataType.includes('monarque_eur'))  // we keep it in case
        wikiData.list.forEach(wikiObj => wikiparser.parseWikiObj(wikiObj))
}
...
*/
wikiparser.parseWikiObj = function (wikiObj) {
    ...
    if(wikiObj.birthDate && wikiObj.deathDate){
        let start = moment(wikiObj.birthDate)
        let end = moment(wikiObj.deathDate)
        
        wikiObj.lifeSpan = moment.duration(end.diff(start)).asYears()
    }
