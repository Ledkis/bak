...
datamanager.saveDataInfoSync = function(dataInfo){
	...
	logger.info(`saveDataInfoSync: ${dataInfoFile} saved`);
}
...
datamanager.saveDataInfo = function (dataInfo){
	...
	.then((writeRes) => {
		logger.info(`saveDataInfo: ${dataInfoFile} saved`);
		return writeRes;
	}).catch((err) => {
...
datamanager.updateDataInfoType = function (dataInfoType, type){
	this.getDataInfo().then((dataInfo) => {
		...
		logger.info(`updateDataInfoType: ${JSON.stringify(dataInfoType)} to update`);
...
datamanager.saveWikiDataRAW = function(wikiDataRaw, opts){
	...
	.then((writeRes) => {
		logger.info(`saveWikiDataRAW: data/raw/${opts.page}.html saved`);
	}).then((res) => {
...
datamanager.saveWikiDataJSON = function(parseDwikiData, opts){
	...
	.then((writeRes) => {
		logger.info(`saveWikiDataJSON: data/json/${parseDwikiData.title}.json saved`);
	}).then((res) => {
